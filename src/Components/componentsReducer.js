...
export default (state = [], action) => {
  ...
  if (['OSCILLATOR_CHANGE_TYPE', 'OSCILLATOR_CHANGE_FREQUENCY', 'OSCILLATOR_CHANGE_TO_MASTER'].includes(action.type)) {
    return state.map(o => oscillatorReducer(o, action));
  }
  if (['FILTER_CHANGE_TYPE', 'FILTER_CHANGE_FREQUENCY', 'FILTER_CHANGE_TO_MASTER'].includes(action.type)) {
